...
class DigiKeyboardDevice : public Print {
 public:
  ...
  size_t write(uint8_t chr) {
	//Caso o ascii seja de teclas terciarias acionados o alt right
	if(chr == 47 || chr == 63 || chr == 162 || chr == 163 || chr == 170 || chr == 172 || chr == 176 || chr == 178 || chr == 179 || chr == 185 || chr == 186 ){
		uint8_t data = pgm_read_byte_near(ascii_to_scan_code_table + (chr - 8));
		sendKeyStroke(data & 0b01111111, data >> 7 ? 0 : MOD_ALT_RIGHT);
	}else{
		uint8_t data = pgm_read_byte_near(ascii_to_scan_code_table + (chr - 8));
